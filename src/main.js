...
scene.fog = new THREE.Fog(0x0a1624, 18, 210);
...
// --- 3. LIGHTING SYSTEM ---
const hemisphereLight = new THREE.HemisphereLight(0xb9cdf4, 0x0b1624, 0.5);
scene.add(hemisphereLight);

// Moon light
const moonLight = new THREE.DirectionalLight(0xc2d8ff, 0.78); 
moonLight.position.set(85, 115, 55);
...
// Cold rim light
const rimLight = new THREE.DirectionalLight(0x2a3955, 0.22);
...
for (let i = 0; i < treeCount; i++) {
    ...
    for (let level = 0; level < levels; level++) {
        ...
        for (let b = 0; b < branches; b++) {
            ...
            for (let c = 0; c < clusterCount; c++) {
                ...
                const snowGeo = new THREE.IcosahedronGeometry(clusterRadius * 0.8, 0);
                snowGeo.translate(
                    tempPosition.x + clusterOffset.x * 0.95,
                    tempPosition.y + clusterOffset.y + clusterRadius * 0.55,
                    tempPosition.z + clusterOffset.z * 0.95
                );
                snowClusters.push(snowGeo);

                const capGeo = new THREE.IcosahedronGeometry(clusterRadius * 1.05, 1);
                capGeo.translate(
                    tempPosition.x + clusterOffset.x,
                    tempPosition.y + clusterOffset.y + clusterRadius * 0.7,
                    tempPosition.z + clusterOffset.z
                );
                snowClusters.push(capGeo);
            }

            if (tempDirection.y > 0.55) {
                const accentRadius = branchRadius * (1.4 + rng() * 0.4);
                const accentGeo = new THREE.IcosahedronGeometry(accentRadius, 0);
                const branchTip = tempDirection.clone().multiplyScalar(length * (0.9 + rng() * 0.15));
                accentGeo.translate(
                    tempPosition.x + branchTip.x,
                    tempPosition.y + branchTip.y + accentRadius * 0.6,
                    tempPosition.z + branchTip.z
                );
    ...
    const snowTint = new THREE.Color().setHSL(0.58, 0.1, 0.9 + rng() * 0.06);
